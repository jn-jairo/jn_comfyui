...
class JN_FirstActive:
    ...
    def run(self, merge_array_items, values=None):
        if values is None:
            values = []
        first_active = None
...
class JN_SelectItem:
    ...
    def run(self, index, only_active, merge_array_items, values=None):
        if values is None:
            values = []
        selected = None
...
class JN_SliceOperation:
    ...
    def run(self, a, b, operation, only_active, merge_array_items, values=None):
        if values is None:
            values = []
        if merge_array_items:
            values = reduce(lambda a, b: (a if isinstance(a, list) else [a]) + (b if isinstance(b, list) else [b]), values, [None])
