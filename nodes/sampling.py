...
class JN_KSampler:
    ...
    def sample(self, model, vae, seed, steps, cfg, sampler_name, scheduler, positive, negative, denoise=1,
        ...
        if image is not None and resize_mask_area_params and resize_mask_area_params["width"] == 0 and resize_mask_area_params["height"] == 0 and resize_mask_area_params["scale_by"] == 0:
            resize_mask_area_params["width"] = image.shape[2]
            resize_mask_area_params["height"] = image.shape[1]

        if resize_mask_area_params:
            if resize_mask_area_params["width"] != 0:
                resize_mask_area_params["height"] = 0
            if resize_mask_area_params["height"] != 0:
                resize_mask_area_params["width"] = 0
    ...
    def tiled_ksampler(self, width, height, overlay_percent, steps_chunk=0, common_ksampler_params=None, seamless_params=None, **kwargs):
        ...
        for step in range(0, steps, steps_chunk):
            ...
            for tile in tiles:
                ...
                logging.info(f"Step: {step}-{step + steps_chunk}/{steps} Tile: {c+1}x{r+1} => {x*8}-{(x+w)*8}:{y*8}-{(y+h)*8} => {w*8}x{h*8}")
